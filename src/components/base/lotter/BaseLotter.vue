...
export default {
  ...
  props: {
    ...
    awards: { type: Array, default: function(){return []}},// 奖品数组
  ...
  methods: {
    ...
    toDraw(e) {
      ...
      const ctx = this.ctx
      // 只要抽奖没有结束，就不让再次抽奖
      if (this.index < 0 || this.index >= this.awards.length) console.error('该索引的奖品不存在!')
      if (!this.canBeClick || this.index < 0 || this.index >= this.awards.length) return false
    ...
    distanceToStop() {
      ...
      // 此次抽奖应该中的奖品
      const currentPrizeIndex = this.index
